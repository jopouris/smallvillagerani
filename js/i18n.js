...
about_feature3: "Προσβάσιμο parking & κοντά σε δραστηριότητες",
      ...
      form_cost:"Υπολογισμός Κόστους",
      form_calc_btn:"Υπολογισμός Κόστους",
      // Contact
...
about_feature3: "Accessible parking & close to activities",
      ...
      form_submit: "Send",
      form_cost:"calculate cost",
      form_calc_btn:"calculate cost",
 ...
 const STORAGE_KEY = "svgerani_lang";
  ...
  /* =======================================================
     3) Εφαρμογή μεταφράσεων σε text & attributes
  ========================================================*/
  ...
  function applyTranslations(lang) {
    ...
    // ενημέρωση toggle button (προβάλλουμε τη γλώσσα που θα μπει)
  ...
  // Εξαγωγή δημόσιου API όταν φορτώσει το DOM (ώστε να υπάρχει σίγουρα)
  document.addEventListener("DOMContentLoaded", () => {
    // Κουμπί toggle
    const btn = document.getElementById("lang-toggle");
    if (btn) btn.addEventListener("click", toggleLang);

    // MutationObserver για δυναμικό περιεχόμενο
    const observer = new MutationObserver((mutations) => {
      for (const m of mutations) {
        m.addedNodes?.forEach(node => {
          ...
          node.querySelectorAll?.("[data-i18n], [data-i18n-attrs]")
            .forEach(el => translateElement(el, currentLang));
        });
      }
    });
...
// ==============================
// 6) Μετά το IIFE (ασφαλές layer)
// ==============================
document.addEventListener("DOMContentLoaded", () => {
  // A) Κάνε όλα τα Google Maps links μεταφράσιμα (εκτός από #mapLink)
  const mapLinks = document.querySelectorAll('a[href*="google.com/maps"]:not(#mapLink)');
  mapLinks.forEach(a => {
  ...
  // B) Επανεφάρμοσε μεταφράσεις χρησιμοποιώντας το public API
  if (window.i18n && typeof window.i18n.applyTranslations === "function") {
    window.i18n.applyTranslations(window.i18n.current);
  }
});