...
// υπολογισμος κοστους 
document.getElementById('calc-cost').addEventListener('click', () => {
  const checkin = new Date(document.getElementById('checkin').value);
  const checkout = new Date(document.getElementById('checkout').value);
  const pricePerNight = 50; // € ανά διανυκτέρευση

  if (checkin && checkout && checkout > checkin) {
    const nights = (checkout - checkin) / (1000 * 60 * 60 * 24);
    const total = nights * pricePerNight;
    document.getElementById('total-cost').value = `${total.toFixed(2)} € (${nights} νύχτες)`;
  } else {
    document.getElementById('total-cost').value = 'Μη έγκυρες ημερομηνίες';
  }
});
